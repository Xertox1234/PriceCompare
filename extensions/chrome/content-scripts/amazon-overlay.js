/**
 * PriceCompare Extension - Amazon Content Script
 * Injects price history chart on Amazon product pages
 */
...
(function () {
  ...
  /**
   * Extract product ASIN from URL or page
   */
  ...
  function findInsertionPoint() {
    ...
    // Fallback: insert after main product info
    return document.querySelector('#centerCol') || document.querySelector('#dp');
  }
  ...
  function renderSummary(history, trend) {
    ...
    const vsAvg = ((currentPrice - avgPrice) / avgPrice) * 100;
    const vsLow = ((currentPrice - lowestPrice) / lowestPrice) * 100;
  ...
  /**
   * Render simple ASCII chart (fallback)
   */
  ...
  async function renderChart(container, productId) {
    try {
      ...
      // Render simple chart (could be replaced with Canvas/SVG chart)
  ...
  async function injectOverlay() {
    ...
    if (!asin) {
      log('Could not extract ASIN from page');
      return;
    }
    ...
    log('Product detected', { asin, title, currentPrice });
    ...
    if (!product) {
      ...
      // Could show a message to user to add this product
      return;
    }
  ...
  async function init() {
    ...
    // Also listen for URL changes (single-page navigation)
