...
export const productOffers = pgTable(
  ...
  (table) => ({
    ...
    productRetailerIdx: index('product_offers_product_retailer_idx').on(
      table.productId,
      table.retailerId
    ),
...
export const priceHistory = pgTable(
  'price_history',
  {
    ...
    productId: integer('product_id')
      .references(() => products.id, { onDelete: 'cascade' })
      .notNull(), // Denormalized for fast queries
    retailerId: integer('retailer_id')
      .references(() => retailers.id, { onDelete: 'cascade' })
      .notNull(), // Denormalized for fast queries
    price: decimal('price', { precision: 10, scale: 2 }).notNull(), // CHECK: >= 0
...
export const notificationPreferences = pgTable('notification_preferences', {
  ...
  priceDropThresholdAmount: decimal('price_drop_threshold_amount', {
    precision: 10,
    scale: 2,
  }).default('5.00'), // $5 default
  // Alert settings
...
export const topicTagRelations = pgTable(
  'topic_tag_relations',
  {
    id: serial('id').primaryKey(),
    topicId: integer('topic_id')
...
export const dealSpottings = pgTable(
  'deal_spottings',
  {
    id: serial('id').primaryKey(),
    userId: integer('user_id')
...
export const priceSnapshots = pgTable(
  ...
  (table) => ({
    ...
    productDateIdx: index('price_snapshots_product_date_idx').on(
      table.productId,
      table.snapshotDate
    ),
  })
 ...
 */
export const priceAggregatesWeekly = pgTable(
  ...
  (table) => ({
    ...
    uniqueProductRetailerWeek: unique('unique_product_retailer_week').on(
      table.productId,
      table.retailerId,
      table.year,
      table.week
 ...
 */
export const priceAggregatesMonthly = pgTable(
  ...
  (table) => ({
    ...
    retailerYearIdx: index('monthly_retailer_year_idx').on(
      table.retailerId,
      table.year,
      table.month
    ),
    createdAtIdx: index('monthly_created_idx').on(table.createdAt),
    uniqueProductRetailerMonth: unique('unique_product_retailer_month').on(
      table.productId,
      table.retailerId,
      table.year,
      table.month
...
export const priceAggregatesDaily = pgTable(
  ...
  (table) => ({
    ...
    uniqueProductRetailerDate: unique('unique_product_retailer_date').on(
      table.productId,
      table.retailerId,
      table.date
    ),
...
export const priceTrends = pgTable(
  ...
  (table) => ({
    ...
    uniqueProductRetailer: unique('unique_product_retailer_trend').on(
      table.productId,
      table.retailerId
    ),
  })
...
export const productSpecifications = pgTable(
  ...
  (table) => ({
    ...
    uniqueProductSpec: unique('unique_product_spec').on(
      table.productId,
      table.specGroup,
      table.specName
    ),
