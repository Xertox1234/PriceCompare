...
export function PriceChangeBadge({ productId, offerId, variant = 'default', className }: PriceChangeBadgeProps) {
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge variant={badgeVariant} className={cn("text-xs cursor-help", className)}>
              <TrendIcon className="mr-1 h-3 w-3" />
              {changePercent > 0 ? '+' : ''}{changePercent.toFixed(1)}% (24h)
            </Badge>
          </TooltipTrigger>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between gap-4">
                  ...
                  <span className={cn("font-medium", isDecrease && "text-green-600", isIncrease && "text-red-600")}>
                ...
                {stats.priceChange7d !== undefined && (
                  <div className="flex justify-between gap-4">
                    ...
                    <span className={cn("font-medium", stats.priceChange7d < 0 && "text-green-600", stats.priceChange7d > 0 && "text-red-600")}>
                      {stats.priceChange7d < 0 ? '' : '+'}{stats.priceChange7d.toFixed(2)} ({stats.priceChangePercent7d?.toFixed(1)}%)
                    </span>
                  </div>
                )}
                {stats.priceChange30d !== undefined && (
                  <div className="flex justify-between gap-4">
                    ...
                    <span className={cn("font-medium", stats.priceChange30d < 0 && "text-green-600", stats.priceChange30d > 0 && "text-red-600")}>
                      {stats.priceChange30d < 0 ? '' : '+'}{stats.priceChange30d.toFixed(2)} ({stats.priceChangePercent30d?.toFixed(1)}%)
                    </span>
                  </div>
                )}
              </div>
              <div className="pt-2 border-t space-y-1 text-sm">
                <div className="flex justify-between gap-4">
                  ...
                  <span className="font-medium text-green-600">${stats.lowestPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between gap-4">
 ...
 */
export function LowestPriceBadge({ productId, offerId, className }: Omit<PriceChangeBadgeProps, 'variant'>) {
  ...
  return (
    <Badge variant="default" className={cn("text-xs bg-green-600", className)}>
 ...
 */
export function PriceTrendIcon({ productId, offerId, className }: Omit<PriceChangeBadgeProps, 'variant'>) {
