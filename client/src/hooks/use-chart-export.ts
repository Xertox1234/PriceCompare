...
export function useChartExport() {
  ...
  const exportAsCSV = useCallback(
    (data: PriceHistoryData[], productName: string, filename?: string): boolean => {
      setIsExporting(true);
      setError(null);
      ...
      try {
        ...
        const rows = data.map((item) => {
          ...
          return [format(date, 'yyyy-MM-dd HH:mm:ss'), item.retailerName, item.price];
        });
   ...
   */
  const copyToClipboard = useCallback(async (elementId: string): Promise<boolean> => {
    setIsExporting(true);
    setError(null);
   ...
   */
  const generateShareLink = useCallback((productId: number, timeRange?: number): string => {
