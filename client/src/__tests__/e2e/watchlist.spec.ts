...
test.describe('Watchlist', () => {
  ...
  test('should add product to watchlist', async ({ page }) => {
    ...
    // Click first product
    const firstProduct = page.locator('[data-testid="product-card"], .product-item, article').first();
    ...
    if (await addButton.isVisible()) {
      ...
      await expect(page.locator('text=/added|watching|success/i').first()).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should display price drop alerts', async ({ page }) => {
    ...
    const priceAlert = page.locator('[data-testid="price-alert"], .price-drop, text=/price drop|alert/i').first();
