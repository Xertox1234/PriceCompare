...
export default function PriceHistoryPage() {
  ...
  const {
    ...
    error: historyError,
  } = usePriceHistory(productId, selectedOfferId, { limit: 500 });

  const { data: stats, isLoading: statsLoading } = usePriceStats(productId, selectedOfferId, 365);

  const { data: _snapshots, isLoading: snapshotsLoading } = usePriceSnapshots(productId, {
  ...
  const insights = stats
    ? {
        volatility: (((stats.highestPrice - stats.lowestPrice) / stats.averagePrice) * 100).toFixed(
          1
        ),
        currentVsAvg: (
          ((stats.currentPrice - stats.averagePrice) / stats.averagePrice) *
          100
        ).toFixed(1),
        bestDealPercentage: (
        ...
        {!isLoading && chartData && chartData.dataPoints.length > 0 && (
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <Badge
                    ...
                    className="mt-2"
                  >
                    ${Math.abs(chartData.currentPrice - chartData.averagePrice).toFixed(2)}{' '}
                    difference
                  </Badge>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">Price data points recorded</p>
                  <Badge variant="secondary" className="mt-2">
                    {chartData.dataPoints.length > 0 && (
                      <>
                        {Math.ceil(
                          (new Date(
                            chartData.dataPoints[chartData.dataPoints.length - 1].date
                          ).getTime() -
                            new Date(chartData.dataPoints[0].date).getTime()) /
                            (1000 * 60 * 60 * 24)
                        )}{' '}
                        days tracked
                      </>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Analysis based on historical data</CardDescription>
                </CardHeader>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground text-sm">
                        Current price is $
                        {(chartData.currentPrice - chartData.lowestPrice).toFixed(2)} from the
                        lowest recorded
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground text-sm">
                        ...
                        {(chartData.highestPrice - chartData.lowestPrice).toFixed(2)} (
                        {insights?.bestDealPercentage}%) by buying at the lowest price
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <PriceInsightsWidget productId={productId} offerId={selectedOfferId} />

              {/* Price Alerts Manager */}
              <PriceAlertsManager productId={productId} currentPrice={chartData.currentPrice} />

              {/* Deal Tracker (full width) */}
              <div className="lg:col-span-2">
                <DealTracker productId={productId} offerId={selectedOfferId} />
              </div>
            </div>
          </div>
        )}
