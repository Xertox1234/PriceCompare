...
test.describe('Price Alert Management', () => {
  ...
  test.beforeEach(async () => {
    // Seed test data
    testProduct = await seedTestData();
  });
 ...
 * Returns the created product ID for test usage
 */
async function seedTestData(): Promise<{ productId: number; productName: string }> {
  ...
  // Seed a small amount of price history so the chart renders dots.
  // The alert modal is triggered by clicking a chart data point.
  const now = new Date();
