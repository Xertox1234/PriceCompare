...
export function createRateLimiter(options: RateLimitOptions) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    // TESTING: Disable rate limiting in E2E test environment
    // This allows E2E tests to run without hitting rate limits
    // Still sets headers for test assertions but with unlimited values
    // Unit/integration tests can explicitly test rate limiting by setting TEST_RATE_LIMITER=true
    // Dedicated rate limiting E2E tests can be added in e2e/rate-limiting.spec.ts
    ...
    if (process.env.NODE_ENV === 'test' && process.env.TEST_RATE_LIMITER !== 'true') {
