...
const priceStatsSchema = z.object({
  days: z
    .string()
    .optional()
    .transform((val) => (val ? parseInt(val) : 90)),
});
...
export function registerPriceHistoryRoutes(app: Express): void {
   ...
   */
  app.get(
    '/api/products/:productId/offers/:offerId/price-stats',
    async (req: Request, res: Response) => {
      try {
        const productOfferId = parseIntSafe(req.params.offerId, 'offerId', { min: 1 });
   ...
   */
  app.post(
    '/api/admin/price-history/record',
    csrfProtection,
    withAdmin(async (req: AuthenticatedRequest, res: Response) => {
      try {
        ...
        const { productOfferId, price, originalPrice, source, confidence, metadata } =
   ...
   */
  app.post(
    '/api/admin/price-history/generate-snapshots',
    csrfProtection,
    withAdmin(async (req: AuthenticatedRequest, res: Response) => {
      try {
   ...
   */
  app.get(
    '/api/admin/price-history/price-drops',
    withAdmin(async (req: AuthenticatedRequest, res: Response) => {
      try {
   ...
   */
  app.delete(
    '/api/admin/price-history/cleanup',
    csrfProtection,
    withAdmin(async (req: AuthenticatedRequest, res: Response) => {
      try {
