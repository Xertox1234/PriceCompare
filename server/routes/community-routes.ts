...
const createWatchListSchema = z.object({
  name: z
    .string()
    .min(1, 'Watch list name is required')
...
const updateProductWatchSchema = z.object({
  ...
  targetPrice: z
    .string()
    .regex(/^\d+(\.\d{1,2})?$/, 'Target price must be a valid decimal')
    .optional()
    .nullable(),
...
const bulkMoveProductsSchema = z.object({
  productWatchIds: z
    ...
    .min(1, 'At least one product watch ID is required')
...
export function registerCommunityRoutes(app: Express) {
  ...
  app.get('/api/community/most-watched', async (req, res) => {
    try {
      const limit =
        parseIntOptional(req.query.limit as string, 'limit', { min: 1, max: 100 }) ?? 10;
