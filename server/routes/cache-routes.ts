...
const cacheWarmingSchema = z.object({
  topProductsCount: z
    ...
    .int()
    .positive()
    .max(10000, 'Top products count must be at most 10000')
    .optional()
    .default(100),
...
export function registerCacheRoutes(app: Express): void {
   ...
   */
  app.get(
    '/api/admin/cache/popularity/products',
    withAdmin(async (req, res) => {
      try {
   ...
   */
  app.get(
    '/api/admin/cache/popularity/searches',
    withAdmin(async (req, res) => {
      try {
        const limit = req.query.limit
          ? parseIntSafe(req.query.limit as string, 'limit', { min: 1, max: 1000 })
   ...
   */
  app.get(
    '/api/admin/cache/popularity/product/:id',
    withAdmin(async (req, res) => {
      try {
  ...
  app.post(
    ...
    withAdmin((req, res) => {
      try {
        ...
        const count = triggerCacheWarming(validatedOptions);
   ...
   */
  app.post(
    '/api/admin/cache/invalidate/product/:id',
    csrfProtection,
    withAdmin(async (req, res) => {
      try {
        const productId = parseIntSafe(req.params.id, 'productId', { min: 1 });
   ...
   */
  app.post(
    '/api/admin/cache/cleanup/popularity',
    csrfProtection,
    withAdmin(async (req, res) => {
      try {
  ...
  app.get(
    ...
    withAdmin(async (req, res) => {
      try {
        ...
        const errorRate =
          stats.cache.overall.totalRequests > 0
        ...
        const allHealthy = Object.values(health.checks).every(
          (check) =>
            check.status === 'healthy' || check.status === 'idle' || check.status === 'active'
        );
