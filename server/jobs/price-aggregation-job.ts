...
export function startPriceAggregationJobs(): void {
  ...
  dailyAggregationJob = cron.schedule(
    '0 1 * * *',
    async () => {
      ...
      const result = await jobLockService.withLock(
        ...
        async () => {
          ...
          logger.info(
            `[PriceAggregationJob] Daily aggregation complete: ${count} aggregates created`
          );
          return count;
        },
        7200 // 2 hour lock
      ...
      if (result === null) {
        logger.info(
          '[PriceAggregationJob] Daily aggregation skipped - already running on another server'
        );
      }
    },
    {
    ...
    async () => {
      ...
      const result = await jobLockService.withLock(
        ...
        async () => {
          ...
          logger.info(
            `[PriceAggregationJob] Weekly aggregation complete: ${count} aggregates created`
          );
          return count;
        },
        7200 // 2 hour lock
      ...
      if (result === null) {
        logger.info(
          '[PriceAggregationJob] Weekly aggregation skipped - already running on another server'
        );
      }
    },
    {
      ...
      if (tomorrow.getDate() === 1) {
        ...
        const result = await jobLockService.withLock(
          ...
          async () => {
            ...
            logger.info(
              `[PriceAggregationJob] Monthly aggregation complete: ${count} aggregates created`
            );
            return count;
          },
          7200 // 2 hour lock
        ...
        if (result === null) {
          logger.info(
            '[PriceAggregationJob] Monthly aggregation skipped - already running on another server'
          );
        }
      }
    },
