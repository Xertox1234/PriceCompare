...
**Status**: ✅ Resolved - Tests Removed (Feature Not Implemented)
...
**Decision**: Tests removed instead of implementing feature

### Why Tests Were Removed

1. **Forum system never implemented**: The codebase contains a complete Discourse-inspired forum schema (13 tables) that was never built out
2. **External forum solution**: User decided to use commercially hosted Discourse for general community forums
3. **Product reviews deferred**: Lightweight product discussion/review system will be implemented later when needed
4. **Technical debt cleanup**: Tests were checking for a feature (`discussionCount`, `hasActiveDiscussion`) that doesn't exist

### Actions Taken

1. ✅ Removed 2 failing test cases from `server/routes/__tests__/product-routes.test.ts`:
   - Line ~317: `it('should include discussion count in results', ...)`
   - Line ~366: `it('should include discussion count', ...)`

2. ✅ Created comprehensive cleanup plan: `.github/ISSUE_TEMPLATE_FORUM_CLEANUP.md`
   - Phase 1: Drop 11 unused forum tables via migration
   - Phase 2: Rename `forumTopics` → `productDiscussions`, `forumPosts` → `discussionReplies`
   - Phase 3: Clean up schema and remove forum-related user profile fields
   - Phase 4-5: Update storage layer and client code

3. ✅ Archived this TODO with resolution notes

### Future Work

**See**: `.github/ISSUE_TEMPLATE_FORUM_CLEANUP.md` for complete forum system cleanup plan

**Key Points**:
- Keep 2 tables for future product reviews: `productDiscussions`, `discussionReplies`
- Delete 11 unused forum infrastructure tables
- Remove 7 forum-related user profile fields (`trustLevel`, `postCount`, etc.)
- Implement product discussion endpoints when user demand grows

### Files Modified

- `server/routes/__tests__/product-routes.test.ts` - Removed 2 test cases (45 tests remaining, down from 47)

### Outcome

✅ Test suite unblocked (2 failing tests removed)
✅ Technical debt documented (forum cleanup plan created)
✅ Future path clear (lightweight product reviews, Discourse for general forums)
